...
router.post('/login', async (req, res) => {
  ...
  // Распаковываем phone и password **до** любых логов или использования
  const { phone, password } = value;

  // Находим пользователя (поле passwordHash помечено как select: false в схеме)
  ...
  // Генерируем JWT, в payload кладём userId, phone и роль
  ...
  // Отдаём токен и данные пользователя
  res.json({
    token,
    user: {
      id: user._id,
